  ...
  for (let i = 0; i < instructions.length; i++) {
    ...
    if (ix.programId === SAGE_PROGRAM_ID) {
      try {
        ...
        if (instructionName === 'Unknown' || !decoded) {
          console.log(`[composite-decoder] Failed to decode SAGE instruction at index ${i}, data: ${ix.data.substring(0, 20)}...`);
        }
        results.push({ index: i, programId: ix.programId, instructionName, decoded, success: true });
      } catch (err: any) {
        console.log(`[composite-decoder] Exception decoding SAGE instruction at index ${i}: ${err.message}, data: ${ix.data.substring(0, 20)}...`);
