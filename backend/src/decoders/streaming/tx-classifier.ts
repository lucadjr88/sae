...
export function classifyTx(tx: any, ctx: ClassifyTxContext): TxMeta {
  ...
  if (operation === 'Unknown') {
    console.log(`[tx-classifier] Unknown operation for tx ${tx.signature?.substring(0, 8)}..., instructions: ${JSON.stringify(tx.instructions?.slice(0, 5))}, hasSAGE: ${tx.programIds?.includes(SAGE_PROGRAM_ID)}`);
  }
