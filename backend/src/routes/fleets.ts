  ...
  try {
    ...
    try {
      ...
      if (!scanTarget) {
        ...
        const rentedList = Array.isArray((scanRes as any)?.rented) ? (scanRes as any).rented : [];
        const ownedList = Array.isArray((scanRes as any)?.owned) ? (scanRes as any).owned : [];
        const existingKeys = new Set((result.fleets || []).map((f: any) => f.key));
        for (const rf of rentedList) {
        ...
        (result as any)._feePayerScan = {
          feePayer: scanTarget,
          rentedFound: rentedList.map((r: any) => ({ key: r.key, label: r.label, owner: r.owner })),
          ownedFound: ownedList.map((r: any) => ({ key: r.key, label: r.label }))
        };
      }
    } catch (e) {
