...
import type { DecodedInstruction, BurnedMaterial, ClaimedItem, MaterialEntry } from '@/services/common';
export function copyToClipboard(text: string, event: MouseEvent | KeyboardEvent): void {
	navigator.clipboard.writeText(text).then(() => {
		...
		if (btn) {
			...
			(btn as HTMLElement).classList.add('copy-link-copied');
			setTimeout(() => {
				(btn as HTMLElement).textContent = originalText;
				(btn as HTMLElement).classList.remove('copy-link-copied');
			}, 1500);
		}
	}).catch(err => {
