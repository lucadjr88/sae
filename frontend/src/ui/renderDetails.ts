...
export function renderCraftingDetailsRows(details: any[], maxDetails: number = 50): string {
  ...
  details.slice(0, maxDetails).forEach(d => {
    ...
    if (txid && txid.includes('+')) {
      ...
      txLink = `<span data-copy class="copy-link" onclick="copyToClipboard('${tx1}', event)" title="Click to copy Start tx">${tx1}...</span> + <span data-copy class="copy-link" onclick="copyToClipboard('${tx2}', event)" title="Click to copy Complete tx">${tx2.substring(0, 8)}...</span>`;
    } else if (txid) {
      txLink = `<span data-copy class="copy-link" onclick="copyToClipboard('${txid}', event)" title="Click to copy tx">${txid}...</span>`;
    }
    ...
    if (opLabel) {
      html += `<tr><td class="op-label-emphasis">${opLabel}</td><td>${material}</td><td>${txLink}</td><td id="${detailsCellId}">${detailsText}</td><td>${feeSol}</td><td>${feeUsd}</td></tr>`;
    } else {
