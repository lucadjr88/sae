...
router.get('/get-wallet-txs', async (req, res) => {
    try {
        ...
        const cutoffH = parseInt(req.query.cutoffH) || 24;
        if (!profileId)
        ...
        for (const w of allowedWallets) {
            ...
            try {
                const txs = await getWalletTxsUtil(w.pubkey, cutoffH, connection);
                console.log(`[get-wallet-txs] wallet=${w.pubkey} txCount=${txs.length}`);
