...
export function decodeInstructions(transactions) {
    const SAGE_PROGRAM_ID = 'SAGE2HAwep459SNq61LHvjxPk4pLPEJLoMETef7f7EE';
    const hasRaw = transactions.length > 0 && transactions[0].raw;
    const txsRaw = hasRaw ? transactions.map((t) => t.raw) : transactions;
    const sageTxs = txsRaw
        .map((tx, i) => ({ tx, i }))
        .filter(({ tx }) => {
        const keys = tx.transaction?.message?.staticAccountKeys;
        const compiled = tx.transaction?.message?.compiledInstructions;
        if (!Array.isArray(keys) || !Array.isArray(compiled))
            return false;
        return compiled.some((ix) => typeof ix.programIdIndex === 'number' && keys[ix.programIdIndex] === SAGE_PROGRAM_ID);
    });
    if (sageTxs.length === 0) {
    ...
    if (binExists) {
        ...
        try {
            ...
            for (let b = 0; b < sageInstructions.length; b += batchSize) {
                ...
                const spawnOpts = { encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 };
                if (useStdin)
                    spawnOpts.input = payload;
    ...
    const results = txsRaw.map((tx, i) => {
        ...
        if (!signature && Array.isArray(tx.signatures) && tx.signatures.length > 0)
            signature = tx.signatures[0];
        if (!signature && tx.raw && tx.raw.signature)
            signature = tx.raw.signature;
        if (!signature && tx.raw && Array.isArray(tx.raw.signatures) && tx.raw.signatures.length > 0)
            signature = tx.raw.signatures[0];
