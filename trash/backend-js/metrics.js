...
const metricsMap = {};
export function recordLatency(url, ms) {
    if (!metricsMap[url])
        metricsMap[url] = { latencies: [], processed: 0, errors: 0, statusCounts: {} };
    metricsMap[url].latencies.push(ms);
    if (metricsMap[url].latencies.length > 100)
        metricsMap[url].latencies.shift();
}
export function getLatency(url) {
    const meta = metricsMap[url];
    if (!meta || meta.latencies.length === 0)
        return 0;
    return meta.latencies.reduce((a, b) => a + b, 0) / meta.latencies.length;
}
export function recordProcessed(url) {
    if (!metricsMap[url])
        metricsMap[url] = { latencies: [], processed: 0, errors: 0, statusCounts: {} };
    metricsMap[url].processed++;
}
export function recordError(url) {
    if (!metricsMap[url])
        metricsMap[url] = { latencies: [], processed: 0, errors: 0, statusCounts: {} };
    metricsMap[url].errors++;
}
export function recordStatusCode(url, statusCode) {
    if (!metricsMap[url])
        metricsMap[url] = { latencies: [], processed: 0, errors: 0, statusCounts: {} };
    if (!metricsMap[url].statusCounts)
        metricsMap[url].statusCounts = {};
    if (!metricsMap[url].statusCounts[statusCode])
        metricsMap[url].statusCounts[statusCode] = [];
    metricsMap[url].statusCounts[statusCode].push(Date.now());
    // keep window recent entries (e.g., last 100)
    if (metricsMap[url].statusCounts[statusCode].length > 500)
        metricsMap[url].statusCounts[statusCode].shift();
}
export function countStatusInWindow(url, statusCode, windowMs) {
    const meta = metricsMap[url];
    if (!meta || !meta.statusCounts || !meta.statusCounts[statusCode])
        return 0;
...
export function shouldExcludeEndpoint(url) {
...
export function getMetrics(url) {
