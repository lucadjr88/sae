...
export async function loadOrCreateRpcPool(profileId) {
...
export async function pruneRpcPool(profileId, force) {
...
export async function pickRpcConnection(profileId, opts) {
    ...
    while (true) {
        for (let i = 0; i < candidates.length; i++) {
            ...
            if (!allowStale && health.isInBackoff(ep.url))
                continue;
            ...
            const release = (r) => {
                ...
                if (r?.success) {
                    ...
                    if (r.latencyMs)
                        metrics.recordLatency(ep.url, r.latencyMs);
