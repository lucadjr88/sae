...
function normalizeFleetForFrontend(fleet, isRented = false) {
    return {
...
export async function saveFleetsAndRented(profileId, fleets, rentedFleets) {
    // Normalize fleets array into the shape expected by the frontend
    const rentedSet = new Set((rentedFleets || []).map(r => r.fleet || r.pubkey || (r.fleetData && r.fleetData.pubkey)).filter(Boolean));
    ...
    const normalizedRented = (rentedFleets || []).map(r => {
        const copy = { ...r };
        if (r.fleetData)
            copy.fleetData = normalizeFleetForFrontend(r.fleetData, true).data;
        return copy;
    });
