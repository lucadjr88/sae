import { inferRecipeName, inferMaterialLabel, normalizeOpName } from "@utils/utils.js";
function renderCraftingDetailsRows(details, maxDetails = 50) {
  ...
  details.slice(0, maxDetails).forEach((d) => {
    ...
    try {
      ...
      if (d.decodedKind && dd) {
        if (d.decodedKind === "process") {
          ...
          if (!detailsText) {
            try {
              detailsText = JSON.stringify(dd).slice(0, 160);
            } catch (e) {
              detailsText = String(dd).substring(0, 160);
            }
          }
          decodedCached = dd;
        }
    ...
    if (opLabel) {
      html += `<tr><td style="font-weight:600">${opLabel}</td><td>${material}</td><td>${feeSol}</td><td>${feeUsd}</td><td>${txLink}</td><td id="${detailsCellId}">${detailsText}</td></tr>`;
    } else {
      html += `<tr><td></td><td>${material}</td><td>${feeSol}</td><td>${feeUsd}</td><td>${txLink}</td><td id="${detailsCellId}">${detailsText}</td></tr>`;
    }
  });
  return html;
}
export {
  renderCraftingDetailsRows
};
